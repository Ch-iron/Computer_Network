        ...
        while(true){
            ...
            //'PUT' command
                ...
                if(Integer.parseInt(requestmessage) == 0){
                    ...
                    for(i = 0; i <= final_seq; i++){
                        Size = (short) ((buffer[i][3] << 8) | buffer[i][4] & 0xFF);
                        if(i == final_seq){
                            if(final_length == 1000){
                                fos.write(buffer[i], 5, 1000);
                            }
                            else{
                                fos.write(buffer[i], 5, (Size % 1000));
                            }
                        }
                        else{
                            fos.write(buffer[i], 5, 1000);
